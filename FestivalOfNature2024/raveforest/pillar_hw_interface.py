...
class Pillar():
    ...
    def __init__(self, port, baud_rate=9600, **kwargs):
    ...
    def cleanup(self):
        print(f"Cleaning up and closing the serial connection for pillar")
        if self.ser.is_open:
            self.ser.close()

    def to_dict(self):
        return dict(
            num_tubes=self.num_tubes, num_sensors=self.num_touch_sensors,
