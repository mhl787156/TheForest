...
const unsigned long period2 = 100;   // Updating lights based on touch sensor status (effect speed)
...
void loop() {
  ...
  // Send serial data alternating each period1 ms
 if (currentMillis - startMillis1 >= period1) {
  ...
  if (currentMillis - startMillis2 >= period2) {
    ...
    for (int i = 0; i < NUM_CAPS; i++) {
      ...
      // Use 'decay' to smooth noisy signals
      if (!capStatus[i]) {
      ...
      if (capStatus[i] || capDecay_LED[i] > 0) {
        ...
        for (int j = 0; j < MAX_LEDS; j++) {
          leds[i][j] = CHSV(tube[i][0], 255, 255);
        }
      }
    }
  }
